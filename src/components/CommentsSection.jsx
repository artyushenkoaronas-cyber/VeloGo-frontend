  ...
  return (
    <div className="flex gap-3 group">
      <div className="relative flex-shrink-0">
        <button
          onClick={() => c.author?.username && navigate(c.author?._id ? `/c/${c.author._id}` : `/c/${c.author?.username}`)}
            ...
            {replies.map(r => {
              ...
              return (
                <div key={r._id} className="flex gap-2 group/reply">
                  <button onClick={() => r.author?.username && navigate(r.author?._id ? `/c/${r.author._id}` : `/c/${r.author?.username}`)}
